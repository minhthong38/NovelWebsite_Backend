...
const getAuthorTaskByUserId = async (userId) => {
  // Tìm AuthorExp trước dựa theo userId
  const authorExp = await AuthorExp.findOne({ idUser: userId });
  if (!authorExp) {
    throw new Error("Không tìm thấy thông tin AuthorExp");
  }

  // Tìm nhiệm vụ đang xử lý (trạng thái pending) hoặc gần nhất
  const currentTask = await AuthorTask.findOne({ idAuthorExp: authorExp._id })
                                      .sort({ updatedAt: -1 });
...
module.exports = { getAuthorTaskById, createAuthorTask,getAuthorTaskByUserId, completeAuthorTask };
