...
const WalletReader = require("../../momo/service/walletUserService")
...
const addUser = async (userData, avatarUrl) => {
  try {
    ...
    await newUser.save();
    await ReaderExpService.createReaderExp(newUser._id);
    await WalletReader.createWallet(newUser._id);
...
const deleteUserById = async (id) => {
  try {
    ...
    if (user) {
      // Xóa dữ liệu liên quan đến Reader
      await ReaderExpService.deleteReaderExp(id);
      await WalletReader.deleteWallet(id);

      // Chỉ xóa dữ liệu tác giả nếu role là 'author'
      if (user.role === 'author') {
        await AuthorExpService.deleteAuthorExp(id);
        await WalletAuthor.deleteWallet(id);
      }
...
const checkUsernameEmail = async (username, email) => {
  try {
    ...
    console.error('Lỗi khi kiểm tra username/email:', error);
